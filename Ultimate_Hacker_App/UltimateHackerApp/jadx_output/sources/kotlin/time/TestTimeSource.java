...
public final class TestTimeSource extends AbstractLongTimeSource {
    ...
    public final void m1628plusAssignLRDsOJo(long duration) {
        long newReading;
        long longDelta = Duration.m1539toLongimpl(duration, getUnit());
        if (longDelta != Long.MIN_VALUE && longDelta != Long.MAX_VALUE) {
            ...
            if ((this.reading ^ longDelta) >= 0 && (this.reading ^ newReading) < 0) {
                m1627overflowLRDsOJo(duration);
            }
        } else {
            double delta = Duration.m1536toDoubleimpl(duration, getUnit());
    ...
    private final void m1627overflowLRDsOJo(long duration) {
        throw new IllegalStateException("TestTimeSource will overflow if its reading " + this.reading + DurationUnitKt.shortName(getUnit()) + " is advanced by " + ((Object) Duration.m1542toStringimpl(duration)) + '.');
    }
}
