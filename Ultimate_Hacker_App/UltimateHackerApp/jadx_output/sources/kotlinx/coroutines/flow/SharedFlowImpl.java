...
public class SharedFlowImpl<T> extends AbstractSharedFlow<SharedFlowSlot> implements MutableSharedFlow<T>, CancellableFlow<T>, FusibleFlow<T> {
    ...
    public List<T> getReplayCache() {
        synchronized (this) {
            ...
            for (int i = 0; i < replaySize; i++) {
                result.add(SharedFlowKt.getBufferAt(buffer, this.replayIndex + i));
            }
            return result;
        }
    }
    ...
    */
    ...
    public boolean tryEmit(T value) {
        ...
        for (Continuation cont : continuationArrFindSlotsToResumeLocked) {
            if (cont != null) {
    ...
    private final Object[] growBuffer(Object[] curBuffer, int curSize, int newSize) {
        ...
        for (int i = 0; i < curSize; i++) {
            SharedFlowKt.setBufferAt(newBuffer, i + head, SharedFlowKt.getBufferAt(curBuffer, i + head));
        }
        return newBuffer;
    }
    ...
    public final Object emitSuspend(T t, Continuation<? super Unit> continuation) {
        ...
        for (Continuation r : continuationArrFindSlotsToResumeLocked) {
            if (r != null) {
                Result.Companion companion2 = Result.INSTANCE;
                r.resumeWith(Result.m167constructorimpl(Unit.INSTANCE));
            }
        }
    ...
    */
    ...
    private final Object tryTakeValue(SharedFlowSlot slot) {
        ...
        for (Continuation resume : continuationArrUpdateCollectorIndexLocked$kotlinx_coroutines_core) {
            if (resume != null) {
                Result.Companion companion = Result.INSTANCE;
                resume.resumeWith(Result.m167constructorimpl(Unit.INSTANCE));
            }
        }
        return value;
    }
        ...
        if (((AbstractSharedFlow) this_$iv).nCollectors != 0 && ($this$forEach$iv$iv = ((AbstractSharedFlow) this_$iv).slots) != null) {
            ...
            while (i < length) {
                ...
                if (element$iv$iv != null) {
                    ...
                    if (cont == null || sharedFlowImpl.tryPeekLocked(slot) < 0) {
                        int resumeCount3 = resumeCount2;
                        resumeCount2 = resumeCount3;
                    } else {
                        if (resumeCount2 >= continuationArr.length) {
                            resumeCount = resumeCount2;
                            int resumeCount4 = continuationArr.length * 2;
                            ?? CopyOf = Arrays.copyOf(continuationArr, Math.max(2, resumeCount4));
