...
public class SemaphoreImpl implements Semaphore {
    ...
    */
    ...
    public final boolean addAcquireToQueue(Waiter waiter) {
        ...
        boolean z3;
        SemaphoreSegment curTail2 = (SemaphoreSegment) tail$FU.get(this);
        ...
        long id$iv = enqIdx2 / SemaphoreKt.SEGMENT_SIZE;
        while (true) {
        ...
        SemaphoreSegment segment = (SemaphoreSegment) SegmentOrClosed.m1730getSegmentimpl(s$iv);
    ...
    private final boolean tryResumeNextFromQueue() {
        ...
        boolean z3;
        SemaphoreSegment semaphoreSegment2 = (SemaphoreSegment) head$FU.get(this);
        ...
        while (true) {
            objFindSegmentInternal = ConcurrentLinkedListKt.findSegmentInternal(semaphoreSegment2, j2, semaphoreImpl$tryResumeNextFromQueue$createNewSegment$1);
            if (SegmentOrClosed.m1732isClosedimpl(objFindSegmentInternal)) {
                j = andIncrement;
                z = false;
                z2 = true;
                break;
            }
            Segment segmentM1730getSegmentimpl = SegmentOrClosed.m1730getSegmentimpl(objFindSegmentInternal);
            while (true) {
                ...
                j = andIncrement;
                if (segment.id >= segmentM1730getSegmentimpl.id) {
                    z3 = true;
                    break;
                }
                if (!segmentM1730getSegmentimpl.tryIncPointers$kotlinx_coroutines_core()) {
                    z3 = false;
                    break;
                }
                if (AbstractResolvableFuture$SafeAtomicHelper$$ExternalSyntheticBackportWithForwarding0.m(atomicReferenceFieldUpdater, this, segment, segmentM1730getSegmentimpl)) {
                    if (segment.decPointers$kotlinx_coroutines_core()) {
                        segment.remove();
                    }
                    z3 = true;
                } else {
                    if (segmentM1730getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
                        segmentM1730getSegmentimpl.remove();
                    }
        ...
        SemaphoreSegment semaphoreSegment3 = (SemaphoreSegment) SegmentOrClosed.m1730getSegmentimpl(objFindSegmentInternal);
        semaphoreSegment3.cleanPrev();
        if (semaphoreSegment3.id > j2) {
            return z;
        }
        int i = (int) (j % SemaphoreKt.SEGMENT_SIZE);
        Object andSet = semaphoreSegment3.getAcquirers().getAndSet(i, SemaphoreKt.PERMIT);
        if (andSet != null) {
            return andSet == SemaphoreKt.CANCELLED ? z : tryResumeAcquire(andSet);
        }
        int i2 = SemaphoreKt.MAX_SPIN_CYCLES;
        for (?? r11 = z; r11 < i2; r11++) {
            if (semaphoreSegment3.getAcquirers().get(i) == SemaphoreKt.TAKEN) {
                return z2;
            }
        }
        return !ChannelSegment$$ExternalSyntheticBackportWithForwarding0.m(semaphoreSegment3.getAcquirers(), i, SemaphoreKt.PERMIT, SemaphoreKt.BROKEN);
    }
