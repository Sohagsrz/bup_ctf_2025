...
class MotionPaths implements Comparable<MotionPaths> {
    ...
    void setView(float position, View view, int[] toUse, double[] data, double[] slope, double[] cycle, boolean mForceMeasure) {
        ...
        if (this.mRelativeToController != null) {
            ...
            float drx = vel[0];
            float dry = vel[c];
            float pos_x = (float) ((rx + (Math.sin(v_y5) * radius)) - (v_width / 2.0f));
            float pos_y = (float) ((ry - (Math.cos(v_y5) * radius)) - (v_height / 2.0f));
            float dpos_x = (float) (drx + (Math.sin(v_y5) * dradius) + (dangle * Math.cos(v_y5) * radius));
            float dpos_y = (float) ((dry - (dradius * Math.cos(v_y5))) + (Math.sin(v_y5) * radius * dangle));
            v_x2 = pos_x;
            if (slope.length >= 2) {
                slope[0] = dpos_x;
                slope[c] = dpos_y;
