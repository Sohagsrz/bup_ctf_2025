...
public class GridLayoutManager extends LinearLayoutManager {
    ...
    void layoutChunk(RecyclerView.Recycler recycler, RecyclerView.State state, LinearLayoutManager.LayoutState layoutState, LinearLayoutManager.LayoutChunkResult result) {
        ...
        while (itemSpanIndex < this.mSpanCount && layoutState.hasMore(state) && remainingSpan > 0) {
            ...
            remainingSpan -= spanSize;
            if (remainingSpan < 0 || (view2 = layoutState.next(recycler)) == null) {
                break;
            }
            consumedSpanCount += spanSize;
            this.mSet[itemSpanIndex] = view2;
            itemSpanIndex++;
        }
        if (itemSpanIndex == 0) {
        ...
        while (i2 < itemSpanIndex) {
            ...
            float otherSize = (this.mOrientationHelper.getDecoratedMeasurementInOther(view3) * 1.0f) / ((LayoutParams) view3.getLayoutParams()).mSpanSize;
        ...
        while (i6 < itemSpanIndex) {
            ...
            if (this.mOrientation != 1) {
                ...
                int right2 = getPaddingLeft() + this.mCachedBorders[this.mSpanCount - params.mSpanIndex];
                right = right2 - this.mOrientationHelper.getDecoratedMeasurementInOther(view6);
                view = view6;
                bottom = bottom3;
                bottom2 = right2;
            } else {
                int left4 = getPaddingLeft() + this.mCachedBorders[params.mSpanIndex];
                int right3 = left4 + this.mOrientationHelper.getDecoratedMeasurementInOther(view6);
                right = left4;
                view = view6;
                bottom = bottom3;
                bottom2 = right3;
