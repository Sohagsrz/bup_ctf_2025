#!/usr/bin/env python3
"""
Final correct implementation with proper magic number calculation
"""
...
def calc_kdata_idx_phase2(r9_val):
    ...
    return ((val - offset) & 0xFFFFFFFFFFFFFFFF) % 24
...
def twist_forward(input_data, key):
    ...
    # Phase 2 - with correct magic number calculation
    r11 = key & 0xFFFF
    r9 = (key >> 8) & 0xFF
    r8 = 0
    for i in range(length):
        kdata_idx = calc_kdata_idx_phase2(r9)
        shift = (r8 * 7) & 0x7
        kdata_byte = (KDATA[kdata_idx] >> shift) & 0xff
        stack[i] ^= kdata_byte
...
if __name__ == "__main__":
    # Test forward/reverse first
    test = b"A" * 29
    print("Testing forward/reverse:")
